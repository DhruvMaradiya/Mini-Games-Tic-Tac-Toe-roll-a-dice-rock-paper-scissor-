...
#play function all button details sizes color commands lambda function which on press 
#arranging the buttons in grid
def play():
...
#press function :- by on click the image embeeds in the selected particuar box
def press(num,r,c):
    global click,count
    if click == True:
...
def checkWin():
	#for checking the winning party
	#the combination of rows and columns for winning
	#1,2,3
	#4,5,6
	#7,8,9
	#1,4,7 
...
def clear():
	#for clearing after the game finishes
    btn1.set('')
    btn2.set('')
    btn3.set('')
    btn4.set('')
